...
Usage:
  1. Observed mode:
     python3 annotate_mge.py --gff <mobilome.gff.gz> --intervals <intervals.txt>

  2. Shuffle mode (stdin stream):
     shuffle -n 10000 <template.gff> <intervals> | python3 annotate_mge.py --gff <mobilome.gff.gz> --stream

Unlike the original annotate_mge.py (which keyword-matched NCBI GFF
product= attributes for terms like "transposase"/"phage"/etc.), this
version treats every feature in MAP's mobilome.gff.gz as an MGE call
directly - no keyword filtering needed, since MAP's output already IS
the MGE prediction (from geNomad, ICEfinder2, IntegronFinder, ISEScan).
"""
...
def parse_mge_features(gff_path):
    """Every feature in MAP's mobilome.gff.gz IS an MGE call - no keyword filtering needed."""
...
def main():
    ...
    parser.add_argument('--gff', required=True, help="Path to MAP's mobilome.gff.gz")
