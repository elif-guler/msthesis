...
echo "dataset,pool,n_fur,n_seqwin,n_fur_only,n_seqwin_only,n_intersection" > "$SUMMARY"
...
for d in "${DATASETS[@]}"; do
  for p in "${POOLS[@]}"; do
    ...
    echo "$d,$p,$n_fur,$n_seqwin,$n_fur_only,$n_seqwin_only,$n_intersection" >> "$SUMMARY"
  done
done
