#!/usr/bin/env python3
# this script uses blast to see if seqwin's markers are a superset of
# fur's markers or not.
...
# get the length of each fur marker
lengths = {}
name = None
length = 0
for line in open(fur_markers):
    if line.startswith(">"):
        if name is not None:
            lengths[name] = length
        name = line[1:].split()[0]
...
# blast all fur markers against seqwin markers in one go
cmd = [
    "blastn",
    "-query", fur_markers,
    "-subject", seqwin_markers,
    "-outfmt", "6 qseqid pident length",
]
result = subprocess.run(cmd, capture_output=True, text=True)
lines = result.stdout.strip().split("\n")

# keep the best (longest) hit per fur marker
best_identity = {}
best_length = {}
for line in lines:
    if line == "":
...
print(len(lengths), "fur markers total")
print(n_found, "found in seqwin markers")
