...
if [[ ! $check ]]; then
    ...
    threshold=$t
    cat targets/* neighbors/* | xargs -n1 basename > valid_ids.txt
    grep -Ff valid_ids.txt scop.out > scopfiltered.out
    r=$(grep '^False' scopfiltered.out)
      if [[ $r != "" ]]; then
            cops -D -d $bdb -r $typeAcc -t $threshold \
                 scopfiltered.out > cops.out
      else
            cp scopfiltered.out cops.out
      fi
    fi
fi
