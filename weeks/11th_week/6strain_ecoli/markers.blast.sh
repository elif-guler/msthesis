...
# markers_blast.sh
#
# For every dataset, checks whether fur's markers.fasta are a subset
# of seqwin's signatures.fasta using blast_markers.py.
...
echo "dataset,n_fur_markers,n_found_in_seqwin,fraction_found" > "$SUMMARY"
...
for d in "${DATASETS[@]}"; do
    ...
    if (cd "$outdir" && python3 "$SCRIPT" > fur_in_seqwin.log 2>&1); then
